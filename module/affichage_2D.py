...
class GUI():
	def __init__(self, env, robot,proxy,couleur_obstacles):
		...
		self.robot = robot
		self.couleur_obstacles = couleur_obstacles
		self.proxy = proxy
		self.window = tk.Tk()
		...
		for objet in self.environnement.objets:
			create_circle(objet.x*mult*scale, objet.y*mult*scale, objet.rayon*mult*scale, self.canvas, couleur_obstacles)
		self.canvas.pack()

	def update(self):
		scale = self.environnement.scale
		if (self.proxy.trace_bool):
